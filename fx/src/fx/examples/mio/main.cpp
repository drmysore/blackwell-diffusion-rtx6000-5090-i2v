...
auto handle_error(std::error_code error) noexcept -> std::int64_t {
  const auto& errmsg = error.message();
  std::println("[fx] [mio] error mapping file: {}, exiting...", errmsg);
  return error.value();
}

auto allocate_file(std::string path, const std::size_t size) noexcept -> void {
...
auto main(int argc, char* argv[]) -> int {
  ...
  for ([[maybe_unused]] auto b : rw_mmap) {
    b += 10;
  }

  // Or just change one value with the subscript operator.
  const std::size_t answer_index = rw_mmap.size() / 2;
  ...
  const std::int64_t the_answer_to_everything = ro_mmap[answer_index];
